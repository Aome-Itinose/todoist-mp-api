...
public class TaskerRepositoryImpl implements TaskerRepository {
    ...
    @Override
    public Optional<Tasker> findByTelegramToken(String telegramToken) {
        String sql = "SELECT * FROM tasker WHERE telegram_token = ?";
        List<Tasker> taskers = jdbcTemplate.query(sql, taskerRowMapper(), telegramToken);

        if (taskers.isEmpty()) {
            return Optional.empty();
        }

        Tasker tasker = taskers.getFirst();
        return Optional.of(tasker);
    }

    @Override
    public boolean existByTodoistToken(String todoistToken) {
        String sql = "SELECT * FROM tasker WHERE todoist_token = ?";
        List<Tasker> taskers = jdbcTemplate.query(sql, taskerRowMapper(), todoistToken);

        return !taskers.isEmpty();
    }

    @Override
    public boolean existByTelegramToken(String telegramToken) {
        String sql = "SELECT * FROM tasker WHERE telegram_token = ?";
        List<Tasker> taskers = jdbcTemplate.query(sql, taskerRowMapper(), telegramToken);
    ...
    @Override
    public boolean existByTelegramAndTodoistToken(String telegramToken, String todoistToken) {
        String sql = "SELECT * FROM tasker WHERE telegram_token = ? AND todoist_token = ?";
        List<Tasker> taskers = jdbcTemplate.query(sql, taskerRowMapper(), telegramToken, todoistToken);

        return !taskers.isEmpty();
    }

    private RowMapper<Tasker> taskerRowMapper() {
        return (rs, _) -> new Tasker(
                rs.getLong("id"),
